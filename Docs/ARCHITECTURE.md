...
> **CP-019 status note (2026-09-21, technical rulings D-043 … D-046).** Build steps 5 and 6
> are implemented for terrain:
> - **DEF-3 is resolved** by P-008 (join-in-progress by ordered snapshots, §4.8).
> - **DEF-1 is resolved for terrain settlement** by P-010 (SQLite ledger, `EntityStore` module).
> - **DEF-6's measurement half and economy policy v1 are resolved** by P-009 and P-010. Placement
>   debit waits for inventory-based building, a GAME decision.
> - The exclusive-writer lease is P-007.
...
| **K9** | `MaterialConfig`: `RGB` (current) vs `SingleIndex` | **`SingleIndex`**, game-owned id↔index table in the adapter, 255 terrain materials max. Nothing visible changes while terrain is on the placeholder grid material; if it later affects how terrain looks, that becomes a GAME decision at that time | **Ruled (D-024); switch deferred by D-045**: ids are exact through the colour table, and the config switch waits for the Director because it changes how terrain looks | 6 |
...
occasional sixth of a second.

**Capture is incremental (D-038, DEF-2 closed).** The cut is taken once at G; chunks are then
read and encoded a few per frame under a budget (`CheckpointPumpMillisPerFrame`, default 2 ms),
and edits keep being admitted, committed and broadcast throughout. **Copy-before-write needs no
copy:** when an edit is about to modify a chunk the capture still owes, the pump captures that
chunk immediately, out of order, from its pre-edit state — which is the property P-003 §4's
dirty banks existed to provide, without a second copy of a 131 KB payload or anything to
reconcile. The hook sits in `Cb.Apply`, immediately before `Backend->ApplyOp`.

The cut stays consistent because a chunk the pump reaches on its own cannot have been edited
since G (any edit would have taken it first), a chunk an edit touches is encoded before
`ApplyOp`, and revisions agree because the hook runs before the revision index advances.
...
The visible hitch at the trigger drops from 0.162 s to 0.035 s, and the part that scales with
the dirty set no longer lands in one frame at all. Publication is still one unbroken step, and
the stall warning now measures exactly that. The 4,096-chunk tail is **reduced, not gone** —
chunk work is spread, so what remains at that size is publication, about sixteen times 0.035 s.
...
**Superseded at CP-019: implemented per [P-008](proposals/P-008-join-in-progress.md), DEF-3
resolved (D-044).** Kept as the original statement of the problem: a multi-chunk operation
...
**unknown; E-1 is the experiment.** *(Answered at CP-019, P-009 §4: against the rendered hole,
+2.4% at the 2 m player dig and within 0.5% from radius 6 up.)* The calibration test: remove a sphere of radius R in
...
| # | Defect | Bound | Status |
|---|---|---|---|
| **DEF-1** | **The commit protocol is not cross-store safe.** Crediting inventory before the journal record is durable leaves ore with no hole; the reverse leaves a hole with no ore, recoverable only while the record is retained. A watermark cannot reconstruct a missing record. Requires: the durable commit point, ack semantics, ordering of every terrain and entity effect, restart reconciliation, retention consumers, disk-error behaviour. Evidence: crash injection before and after every write, flush, entity transaction, ack and segment deletion, including transfer/spend after mining | 4 | **Resolved for terrain settlement, CP-019 (D-046, P-010)** — journal first; intent recorded in the record; idempotent SQLite ledger keyed by (OpSeq, digest); boot settles (W,H]; kill-tested. Non-terrain entity transactions do not exist yet |
| **DEF-2** | **Snapshot data and revision metadata have no atomic publication point.** A capture can observe post-edit data before the metadata commit and label it with the preceding revision; replay then applies an op already baked in. Overwriting a snapshot leaves no previous recoverable generation. Requires: a consistent capture boundary, crash-safe publication, the source of truth for the next global sequence, behaviour when mutation succeeds but persistence fails | 4 | **Resolved 2026-09-20** — each clause, in order. *Consistent boundary:* the cut is taken at G and copy-before-write encodes any chunk an edit is about to change, from its pre-edit state, before `ApplyOp` — so a capture cannot observe post-edit data, and the revision encoded is the one held at G because the hook runs before the revision index advances (D-038, tested by `Persistence.Capture.Pump`). *Crash-safe publication:* payloads, index pages and descriptor go into one pack flushed strictly before the root slot, and the root is a two-slot pair with generations, so the previous checkpoint stays recoverable until the new one is named (P-004 §12, §13; D-034, D-036). *Next global sequence:* the journal head H is the source of truth and is seeded into the edit queue at boot, which is what stopped sequence reuse after the first restart. *Mutation succeeds but persistence fails:* a failed commit closes admission and the world must restart from disk; a failed **checkpoint** disables further captures and keeps playing, because the previous root and the journal are both intact (D-033) |
| **DEF-3** | **Per-chunk JIP can replay an operation into a chunk that already contains it.** A multi-chunk op where one chunk's snapshot precedes it and another's includes it applies twice; arrival order changes the result. A global "already applied" flag is insufficient. Requires: application scope, read halos or a coordinated baseline set, duplicate handling, a finite sync cut, atomic sync/live handoff, stale-fragment discrimination, cancellation and backpressure | 5 | **Resolved, CP-019 (D-044, P-008)** — serialized commits plus one ordered channel per client; per-chunk synced set; pointwise kernel |
